        ...
        for (; *cp; cp++) {
            if ((*cp=='%') && cp[1]) {
                ...
                case 's': { // ssh params
                        ...
                        cmdbuf.appendf("%s -v -o StrictHostKeyChecking=%s -o BatchMode=yes ",usepssh?"pssh":"ssh",strict?"yes":"no");
...
public:
    ...
    void exec(unsigned i,unsigned treefrom)
    {
        ...
        try {
            ...
            if (cmdline.length()==0) {
                // ssh
                cmdline.appendf("%s -n -v -o StrictHostKeyChecking=%s ",usepssh?"pssh":"ssh",strict?"yes":"no");
                if (!usepssh)
