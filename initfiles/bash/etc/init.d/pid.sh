...
createPid () {
    ...
    if [[ $__flagPid -eq 1 ]]; then
        [[ ${DEBUG:-NO_DEBUG} != "NO_DEBUG" ]] && log_failure_msg "Pid file already exists"
        ...
        checkPid ${PIDFILEPATH}
        if [[ $__flagPid -eq 1 ]]; then
            [[ ${DEBUG:-NO_DEBUG} != "NO_DEBUG" ]] && log_success_msg
            __pidCreated=1
        else
            [[ ${DEBUG:-NO_DEBUG} != "NO_DEBUG" ]] && log_failure_msg "Failed to create Pid"
...
removePid () {
    ...
    if [[ $__flagPid -eq 0 ]]; then
        [[ ${DEBUG:-NO_DEBUG} != "NO_DEBUG" ]] && log_failure_msg "Pidfile doesn't exist"
        ...
        if [[ ! -e ${PIDFILEPATH} ]]; then
            ...
            [[ ${DEBUG:-NO_DEBUG} != "NO_DEBUG" ]] && log_failure_msg "Failed to remove pid"
...
check_status() {
    ...
    if [[ $componentLocked -eq 1 ]] && [[ $initRunning -eq 1 ]] && [[ $compRunning -eq 1 ]]; then
        [[ ${DEBUG:-NO_DEBUG} != "NO_DEBUG" ]] && echo "everything is up except sentinel"
        log "$compName ---> Waiting on Sentinel"
        if [[ ${SENTINELFILECHK} -eq 1 ]]; then
            if [[ ${sentinelFlag} -eq 0 ]]; then
                [[ ${DEBUG:-NO_DEBUG} != "NO_DEBUG" ]] && echo "Sentinel not yet located, process currently unhealthy"
                log "$compName ---> Currently Unhealthy"
                return 2 
            fi
            [[ ${DEBUG:-NO_DEBUG} != "NO_DEBUG" ]] && echo "Sentinel is now up"
    ...
    # check if shutdown and healthy
        ...
        if [[ ${SENTINELFILECHK} -eq 1 ]]; then
            if [[ ${sentinelFlag} -eq 1 ]]; then
                [[ ${DEBUG:-NO_DEBUG} != "NO_DEBUG" ]] && echo "Sentinel is up but orphaned"
                log "$compName ---> Orphaned State"
                return 3
            fi
            [[ ${DEBUG:-NO_DEBUG} != "NO_DEBUG" ]] && echo "Sentinel is now down"
