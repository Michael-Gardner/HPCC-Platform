...
check_status() {
    ...
    # check if shutdown and healthy
      ...
      [ $pidfilepathExists -eq 0 ]     && log "$compName ---> pid file path does not exist: $1"
      [ $comppidfilepathExists -eq 0 ] && log "$compName ---> comp pid file path does not exist: $3"
      [ $componentLocked -eq 0 ]       && log "$compName ---> component is not locked: $2"
      [ $initRunning -eq 0 ]           && log "$compName ---> process for ${compName}_init.pid is not running"
