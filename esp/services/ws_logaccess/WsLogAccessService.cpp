...
struct LogAccessConditions
    ...
    Owned<IPropertyTree> testTree = createPTreeFromXMLString(
    		        ...
    		        "   </Fields>"
    		        "   <Filters>"
    		        "    <Filter primary='true' name='hpcccontainer'><Field name='container.image.name'/><Operator value='is'/><Value value='hpccplatform-core'/>"
    		        "    </Filter>"
    		        "   </Filters>"
    		        " </LogMap>"
                    "</logaccess>");
    ...
    StringBuffer thexml2;
        toXML(m_logMap, thexml2,0,0);
        DBGLOG("^^^^^5555^%s", thexml2.str());
...
bool Cws_logaccessEx::onGetWULog(IEspContext &context, IEspGetWULogRequest &req, IEspGetWULogResponse &resp)
{
	...
	m_logAccess->fetchWULog(logcontent, "W202104004-454545", range, &cols);
